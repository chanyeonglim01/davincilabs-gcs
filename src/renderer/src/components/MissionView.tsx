...
// ─── Drone icon: drone_icon.svg + heading line (128×128, anchor at center=drone body center) ──
...
export function MissionView() {
  ...
  useEffect(() => {
    ...
    if (surveyPolygonClosed) {
      // Filled polygon
      surveyPolygonRef.current = L.polygon(
        surveyVertices.map((v) => [v[0], v[1]] as L.LatLngTuple),
        {
          color: '#FFD740',
      ...
      surveyEdgeRef.current = L.polyline(
        surveyVertices.map((v) => [v[0], v[1]] as L.LatLngTuple),
        {
          color: '#FFD740',
          weight: 1.5,
  ...
  useEffect(() => {
    ...
    const pts = surveyPreviewResult.waypoints.map((wp) => [wp.lat, wp.lon] as L.LatLngTuple)
  ...
  const handleDrop = (e: React.DragEvent, targetUid: number) => {
    ...
    if (fromUid === null || fromUid === targetUid) {
      setDropTarget(null)
      return
    }
    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          />
          <span style={{ fontFamily: mono, fontSize: '11px', color: 'rgba(236,223,204,0.35)' }}>
            m
          </span>
          <button
            onClick={() =>
              setWaypoints((prev) =>
                prev.map((w) => (ACTIONS[w.action].hasAlt ? { ...w, alt: defaultAlt } : w))
              )
            }
            style={{
              fontFamily: mono,
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, position: 'relative' }}>
          ...
          >
            <button
              onClick={() => switchTo2D('satellite')}
              style={mapBtnStyle(mapMode === '2d' && tileMode === 'satellite')}
            >
              SAT
            </button>
            <button
              onClick={() => switchTo2D('dark')}
              style={mapBtnStyle(mapMode === '2d' && tileMode === 'dark')}
            >
              2D
            </button>
            <button onClick={switchTo3D} style={mapBtnStyle(mapMode === '3d')}>
              3D
            </button>
          </div>
        </div>
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', gap: '2px' }}>
                      <button
                        ...
                        style={{ ...miniBtn, opacity: idx === 0 ? 0.2 : 0.6 }}
                        disabled={idx === 0}
                      >
                        ↑
                      </button>
                    ...
                    <button
                      ...
                      style={{ ...miniBtn, color: '#E87020', opacity: 0.7 }}
                    >
                      ✕
                    </button>
                  </div>
                    ...
                    >
                      <div style={{ display: 'flex', gap: '6px' }}>
                        <CoordInput
                          ...
                          value={wp.lat}
                          step={0.00001}
                          onChange={(v) => updateWp(wp.uid, { lat: v })}
                        />
                        <CoordInput
                          label="LON"
                          value={wp.lon}
                          step={0.00001}
                          onChange={(v) => updateWp(wp.uid, { lon: v })}
                        />
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                        ...
                        <input
                          type="number"
                          value={wp.acceptRadius}
                          min={1}
                          max={500}
                          step={1}
                          onChange={(e) =>
                            updateWp(wp.uid, { acceptRadius: Number(e.target.value) })
                          }
                      ...
                      {wp.action === 'LOITER' && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                          ...
                          <input
                            type="number"
                            value={wp.loiterRadius}
                            min={10}
                            max={2000}
                            step={10}
                            onChange={(e) =>
                              updateWp(wp.uid, { loiterRadius: Number(e.target.value) })
                            }
                            style={{ ...smallInput, width: '56px', textAlign: 'right' }}
                          />
                          <span
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
      ...
      <span style={{ fontFamily: mono, fontSize: '13px', fontWeight: 700, color: '#ECDFCC' }}>
        {value}
      </span>
    </div>
  )
...
function CoordInput({
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '2px' }}>
      <span style={{ fontFamily: sans, fontSize: '9px', color: 'rgba(236,223,204,0.35)' }}>
        {label}
      </span>
      <input
        type="number"
          ...
          {data.length < 1 ? null : (
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={data} margin={{ top: 14, right: 24, bottom: 4, left: 44 }}>
                ...
                <Tooltip
                  content={<AltTooltip />}
                  cursor={{ stroke: 'rgba(79,195,247,0.3)', strokeWidth: 1 }}
                />
...
function WaypointDot(props: { cx?: number; cy?: number; payload?: AltPoint }) {
