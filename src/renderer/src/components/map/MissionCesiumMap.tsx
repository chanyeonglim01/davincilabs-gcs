...
export const MissionCesiumMap = forwardRef<MissionCesiumMapHandle, MissionCesiumMapProps>(
  function MissionCesiumMap(
    { initialCenter, waypoints, selectedUid, onAddWaypoint, onSelectWaypoint, onMoveWaypoint },
    ref
  ) {
    ...
    useEffect(() => {
      ...
      const cartographics = navWps.map((wp) => Cesium.Cartographic.fromDegrees(wp.lon, wp.lat))
    ...
    }, [waypoints, selectedUid, terrainLoaded])
    ...
    useImperativeHandle(ref, () => ({
      getCameraState: () => {
        ...
        const zoom = Math.max(
          1,
          Math.min(20, Math.round(15 - Math.log2(Math.max(300, alt) / 2500)))
        )
        return { lon, lat, zoom }
      }
    }))
